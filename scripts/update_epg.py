...
def run():
    ...
    for src in sources:
        ...
        for cdata in chan_map.values():
            search_db.append({"id": cdata['id'], "n": cdata['n'], "l": cdata['l'], "c": src['name'], "f": filename, "p": cdata['p'][:3]})
            if cdata['p']:
        ...
        if country_db:
            ...
            final_countries.append({"name": src['name'], "slug": country_slug, "file": filename, "count": len(country_db), "updated": src['age'], "external": src.get('is_external', False)})
